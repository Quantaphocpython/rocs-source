...
function getClassIcon(className: Class) {
  switch (className) {
    ...
    case Class.WATER:
      return <Droplet className="w-4 h-4 text-blue-700" />;
    case Class.WOOD:
      return <Trees className="w-4 h-4 text-green-700" />;
    case Class.EARTH:
      return <Mountain className="w-4 h-4 text-yellow-700" />;
    case Class.METAL:
      return <Cog className="w-4 h-4 text-gray-700" />;
  }
}
        ...
        <div className="p-6 space-y-6 bg-gradient-to-b from-gray-800 to-gray-900">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {effects.map((effect, index) => (
                  <div
                    key={index}
                    className="p-3 rounded-md bg-gray-700/50 border border-gray-600"
                  >
          ...
          <div className="p-4 rounded-md bg-gray-700/50 border border-gray-600">
            ...
            <div className="space-y-1 text-xs text-gray-300">
              ...
              {card.health > 7 && (
                <p>• High health makes it ideal for defensive positions</p>
              )}
              {card.staminaCost < 3 && (
                <p>• Low cost allows for flexible timing</p>
              )}
              {effects.length > 0 && (
                <p>• Combine effects with other cards for maximum impact</p>
              )}
            </div>
          </div>
        </div>
